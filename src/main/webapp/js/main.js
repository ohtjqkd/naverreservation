...
function ajax(requestType, api, param){
	urlString = api + param
	$.ajax({
		type: requestType,
		url: urlString,
		...
		success: function(response){
			...
			for(var key in response){
				if(key.search("Response") != -1){
					len = response[key].items.length*100;
					console.log(len);
					all(".visual_img")[0].style.cssText = `width:calc(${len}%);`;
					templating(response[key], "#promotionItem", ".visual_img")
					return response[key];
				}
			}
		}
...
//		async:false,
		...
		success: function (response) {
			...
			var productTemplate = $("#itemList").html();
			...
			if(startIndex == 0){
				$(".lst_event_box:nth-of-type(1)").html(leftTemplate); 
				$(".lst_event_box:nth-of-type(2)").html(rightTemplate); 
			} else {
				$(".lst_event_box:nth-of-type(1)").append(leftTemplate); 
				$(".lst_event_box:nth-of-type(2)").append(rightTemplate); 
			}
			$(".pink").html(categoryCount + "개");
			
		}
	});
}
function slide(){
	const wrap = all(".visual_img");
	const target = wrap[0];
...
function templating(response, id, targetQuery){
	var template = document.getElementById(id);
	var bindTemplate = Handlebars.compile($(id).html());
	var resultTemplate = response.items.reduce(function(p,c){
		return p += bindTemplate(c);
	},"");
	$(targetQuery).html(resultTemplate);
}
...
function changeTab(e){
	...
	var aElements = all(".anchor");
	if(e.target.tagName=="SPAN"){
		aElements.forEach(function(element,i){
		...
		startIndex = 0;
		categoryId = e.target.parentNode.parentNode.getAttribute('data-category');
...
$(".event_tab_lst").click(function(e){
	changeTab(e);
	buttonController();
});
...
//			$("#promotion>ul").html(resultTemplate);
